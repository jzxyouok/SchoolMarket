		...
		if (gettype.getSelectedItem().equals("--所有产品--")) {
			...
			// 对SellInfo ProductInfo表进行联合查询
	 		if (!SellModel.check("select count(*) from ProductInfo where (Pid like '%'+?+'%' or PName like '%'+?+'%') and Ptype = ?", newtiaojianparas)) {
				...
				JOptionPane.showMessageDialog(this, "<html><br/><font size = '5'>在产品类别：<font color = 'red'>"+gettype.getSelectedItem()+"</font>中<br/>没有找到与：<font color = 'red'>"+tiaojian+"</font>&nbsp&nbsp相关的产品信息</font><br/><br/>");
				return;
